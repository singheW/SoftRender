...
Vec3f eye(1, 1, 3);
Vec3f center(0, 0, 0);
Vec3f light_dir = Vec3f(1, -1, 1).normalize();//Vec3f light_dir(0, 0, 1);//假设光是垂直屏幕的
Vec3f m2v(Matrix m) {
...
Matrix lookat(Vec3f eye, Vec3f center, Vec3f up) {//在新坐标系中进行render
	...
	for (P.x = bboxmin.x; P.x <= bboxmax.x; P.x++) {
		for (P.y = bboxmin.y; P.y <= bboxmax.y; P.y++) {
			...
			if (zbuffer[int(P.x + P.y * width)] < P.z) {
				...
				TGAColor vcolor (255, 255, 255, 255); //diffuse.get(int(indx * 1024), int(indy * 1024));去掉纹理
				//TGAColor vcolor;
				vcolor.r *= intensity;
				vcolor.g *= intensity;
				vcolor.b *= intensity;
...
int main(int argc, char** argv) {
	...
	Projection[3][2] = -1.f / (eye - center).norm();// camera.z;//  float norm() { return std::sqrt(x * x + y * y + z * z); }
	...
	for (int i = 0; i < model->nfaces(); i++) {
		...
		for (int j = 0; j < 3; j++) {
			...
			pts[j] = m2v(ViewPort * Projection *ModelView* v2m(v));//world2screen(model->vert(face[i]));////世界坐标系：升维，投影变换，视窗变换，降维：视窗坐标系
			world_coords[j] = v;
