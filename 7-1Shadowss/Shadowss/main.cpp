...
struct Shader : public IShader {//用抽象的类IShader作为着色器和光栅器之间的一个中间件。
	...
	mat<4, 4, float> uniform_Mshadow;//定义一个矩阵：将当前视角的屏幕坐标参数，转换为shadow map中对应位置的坐标参数，即将像素点投影到光源上并比较z轴深度，从而判定是否存在硬阴影。

	Shader(Matrix M, Matrix MIT, Matrix MS) : uniform_M(M), uniform_MIT(MIT), uniform_Mshadow(MS), varying_uv(), ndc_tri() {}
   ...
   // Vec3f varying_intensity; // written by vertex shader, read by fragment shader
	...
	virtual bool fragment(Vec3f bar, TGAColor& color) {
		Vec4f sb_p = uniform_Mshadow * embed<4>(ndc_tri * bar);//ndc_tri提供了我们当前绘制的像素的屏幕坐标，//计算光源视角转换后坐标
		sb_p = sb_p / sb_p[3];//降维
		int idx = int(sb_p[0]) + int(sb_p[1]) * width;//计算shadow map坐标
		float shadow = .3 + .7 * (shadowbuffer[idx] < sb_p[2] + 43.34 );//知道 xyz 在阴影缓冲空间中的坐标。要确定当前像素是否亮起，只需将其 z 坐标与存储在阴影缓冲器中的值进行比较就足够了。
		// + 43.34 避免Z-fighting ，当shadow map中的深度值小于当前像素投影的z轴值的时候，括号内为真，由于z轴距离相机越远则越小，所以括号内为真表示光线能够照射到，否则不能且为0。
	   ...
	   // Vec3f n = proj<3>(uniform_MIT * embed<4>(model->normal(uv))).normalize();//获取法线  
		...
		for (int i = 0; i < 3; i++) color[i] = std::min<float>(20 + color[i] * shadow * (1.2 * diff + .6 * spec), 255);//对环境分量取5，漫射分量取1，镜面分量取0.6。 std::min<float>使值不超过255
...
struct DepthShader : public IShader {
	...
	virtual bool fragment(Vec3f bar, TGAColor& color) {
		...
		color = TGAColor(255, 255, 255) * (p.z / depth);//仅存储Z轴大小
		return false;
	}
};
int main(int argc, char** argv) {
	...
	light_dir.normalize();//归一化光照向量
	{
		...
		lookat(light_dir, center, up);//从光源方向得到深度图
	...
	Matrix M = Viewport * Projection * ModelView;	//矩阵 M 是从对象空间到阴影缓冲屏空间的转换矩阵。
	{
		...
		// 
		//将对象的坐标转换为（帧缓冲器）屏幕空间。我们需要知道如何将帧缓冲器屏幕转换为阴影屏幕。它非常简单：允许将帧缓冲坐标转换为对象坐标，然后在帧缓冲器和阴影缓冲器之间进行转换。
		Shader shader(ModelView, (Projection * ModelView).invert_transpose(), M * (Viewport * Projection * ModelView).invert());
		Vec4f screen_coords[3];

		//Matrix M = Viewport * Projection * ModelView;//M为pass1中的MVP，
		//uniform_Mshadow = M*(Viewport*Projection*ModelView).invert()；//左乘pass2的MVP并求逆变换之后，就成了阴影变换矩阵，这
		//是因为世界坐标系在pass2点着色中MVP变换为了相机视角坐标，pass2处将相机视角坐标，左乘MVP逆矩阵后就回到了世界坐标系，再左乘光源视角MVP变换，就形成了光源视角坐标。
		//Mshadow它的作用是将当前视角的屏幕坐标参数，转换为shadow map中对应位置的坐标参数，即将像素点投影到光源上并比较z轴深度，从而判定是否存在硬阴影。
		// 
