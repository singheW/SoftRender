...
void viewport(int x, int y, int w, int h) {//视窗变换，将屏幕图像根据显示图像窗口的大小和中心位置进行调整。
    ...
    Viewport[2][3] = depth/2.f;//修改值为depth
    Viewport[0][0] = w/2.f;
    Viewport[1][1] = h/2.f;
    ...
    for (P.x=bboxmin.x; P.x<=bboxmax.x; P.x++) {
        for (P.y=bboxmin.y; P.y<=bboxmax.y; P.y++) {
          ...
          //  int frag_depth = std::max(0, std::min(255, int(z/w+.5)));
            if (c.x<0 || c.y<0 || c.z<0 || zbuffer[P.x + P.y * image.get_width()]>frag_depth) continue;//zbuffer图片r中存储值比当前值深，不动。(距屏幕浅在前)
            bool discard = shader.fragment(c, color);
            if (!discard) {//如果discard 返回false，则if(true)//当确定这个像素需要进行着色的时候，才会调用片段着色器fragment shader进行着色。
                zbuffer[P.x + P.y * image.get_width()] = frag_depth;//frag_depth画浅的
