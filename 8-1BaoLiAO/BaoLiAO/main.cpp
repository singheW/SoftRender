...
struct ZShader : public IShader {//pass1着色器代码，将随机视角的深度缓存。
...
struct Shader : public IShader {//pass2着色器代码，比较观察视角和这一次采样光源的深度缓存，将非阴影部分存储在一次环境光纹理occl中。
	...
	virtual bool fragment(Vec3f gl_FragCoord, Vec3f bar, TGAColor& color) {
		...
		if (std::abs(shadowbuffer[int(gl_FragCoord.x + gl_FragCoord.y * width)] - gl_FragCoord.z < 1e-2)) {
			occl.set(uv.x * 1024, uv.y * 1024, TGAColor(255));//occl最初是清晰的图像。这行代码告诉我们，如果片段是可见的，那么我们使用片段的纹理坐标在该图像中放置一个白点。
		}
...
//对每一个采样点，都会进行两次shader，第一次shader将shadow map存在framebuffer中，第二次shader将shadow map中比较得到的可见部分像素存在occl中。
//每一次环境光照信息纹理存储在occl中，每一遍得到的叠加纹理存储在occlusion中。
...
int main(int argc, char** argv) {
	...
	AOShader aoshader;
	aoshader.aoimage.read_tga_file("obj/occlusion.tga");
